...
async function handle(command) {
  ...
  if (text.startsWith('go ')) {
    ...
    lines.forEach((line, index) => {
      ...
      post('info depth 1 multipv ' + (index + 1) + ' score cp ' + Math.round(line.score || 0) + ' pv ' + line.moves.join(' '));
    });
    post('bestmove ' + (result.bestmove || lines[0]?.moves?.[0] || '0000'));
    return;
  }
  if (text === 'ucinewgame') {
...
self.onmessage = event => {
  ...
  queue = queue.then(() => handle(command)).catch(error => {
    post('info string Maia worker error: ' + (error?.message || error));
  });
};
